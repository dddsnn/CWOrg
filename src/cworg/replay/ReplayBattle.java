...
public class ReplayBattle {
	...
	/**
	 * Returns whether all players from either team are from the same clan
	 * @return
	 */
	public boolean isSameClanBattle() {
		String tag1 = team1.firstElement().getClanTag(), tag2 =
				team2.firstElement().getClanTag();
		for (ReplayBattlePlayer p : team1) {
			if (!p.getClanTag().equals(tag1) || tag1.equals(""))
				return false;
		}
		for (ReplayBattlePlayer p : team2) {
			if (!p.getClanTag().equals(tag1) || tag2.equals(""))
				return false;
		}
		if (tag1.equals(tag2))
			return false;
